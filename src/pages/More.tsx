...
const MENU_GROUPS: { title: string; items: { icon: React.ReactNode; label: string; desc: string; to?: string }[] }[] = [
  {
    title: '旅行工具',
    items: [
      { icon: <Coins size={18} />, label: '匯率換算', desc: '日幣／台幣即時換算', to: '/converter' },
      { icon: <CloudSun size={18} />, label: '天氣預報', desc: '每日天氣與穿搭建議', to: '/weather' },
      ...
      { icon: <BarChart3 size={18} />, label: '統計分析', desc: '旅行結束後的完整摘要', to: '/statistics' },
    ],
  },
]
          ...
          {authEnabled && (
            <Card padded={false} className="mt-3">
              <button
                ...
                className="flex w-full items-center gap-3 px-4 py-3.5 text-left active:bg-cream-soft/60 dark:active:bg-dusk-bg/60"
              >
                <span className="flex h-9 w-9 items-center justify-center rounded-full bg-apricot/15 text-apricot">
                  <LogOut size={18} />
                </span>
                <span className="flex-1">
                  <span className="block text-[14px] text-ink dark:text-cream-soft">登出</span>
                  <span className="block text-[12px] text-warmgray dark:text-warmgray-light">回到登入畫面</span>
                </span>
              </button>
            </Card>
          )}
