...
interface SortableItineraryCardProps {
  ...
  onOpenActions: (item: ItineraryItem) => void
}

function SortableItineraryCard({ item, onToggleComplete, onOpenActions }: SortableItineraryCardProps) {
    ...
    >
      <motion.button
        ...
        className={`relative z-10 flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-sm shadow-card transition-colors ${
          item.completed ? 'bg-sage text-cream-card' : 'bg-cream-card dark:bg-dusk-card'
        }`}
        aria-label="切換完成狀態"
      >
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <span
              ...
              aria-label="拖曳排序"
              className="cursor-grab touch-none p-1 text-warmgray/40 active:cursor-grabbing"
            >
...
export default function Itinerary() {
  const { days, loading: tripLoading, updateDay } = useTrip()
  ...
  function openAreaEdit() {
    setAreaDraft(currentDay?.areaLabel ?? '')
    setAreaEditOpen(true)
  }

  function handleAreaSave() {
    if (!currentDay) return
    updateDay(currentDay.id, { areaLabel: areaDraft.trim() || undefined })
    setAreaEditOpen(false)
    show('區域標籤已更新', 'success')
  }
  ...
  return (
    <div className="mx-auto max-w-lg pb-24">
        ...
        {currentDay && (
          <div className="mb-4 flex items-center gap-2 text-[13px] text-warmgray dark:text-warmgray-light">
            ...
            <button
              ...
              className="flex items-center gap-1 rounded-pill bg-sage-light/50 px-2.5 py-0.5 text-sage-dark active:bg-sage-light/80 dark:bg-sage-dark/30 dark:text-sage-light"
            >
              <MapPinIcon size={11} />
              {currentDay.areaLabel || '設定區域'}
      ...
      {/* 區域標籤編輯 */}
      <BottomSheet open={areaEditOpen} onClose={() => setAreaEditOpen(false)} title="設定今日區域標籤">
        <div className="space-y-4">
          <div>
            <label className="mb-1.5 block text-[12px] text-warmgray dark:text-warmgray-light">區域名稱</label>
            <input
              autoFocus
              value={areaDraft}
              onChange={(e) => setAreaDraft(e.target.value)}
              placeholder="例如：淺草・晴空塔"
